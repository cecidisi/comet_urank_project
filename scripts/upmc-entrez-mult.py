...
def fetch_worker(base_url, query_key, web_env, retmax, batches, i):
	...
	while(cur_attempt <= max_attempts and len(items) == 0):
		...
		cur_attempt += 1
	print 'Batch (' + str(i+1) + '/' + str(batches) + ') --> Got ' + str(len(items)) + ' items ('+str(cur_attempt-1)+' attempt/s) [retstart = ' + str(retstart) + ', retmax = ' + str(retmax) +']'
	articles = []
	for p in items:
		paper = parse_paper(p)
...
def fetch_parallel(base_url, query_key, web_env, retmax, count, cores):
	...
	tmsp2 = time.time()
	# print_green('Time Lapse = ' + str(tmsp2) + ' sec.')
	# jobs is list of lists
	papers = list(chain.from_iterable(jobs))
	print_green('Total papers fetched = ' + str(len(papers)) + ', Time elapsed = ' + str(tmsp2-tmsp1) + ' sec.')
